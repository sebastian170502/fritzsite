...
import { ArrowLeft, Loader2, Truck, Package, MapPin, ExternalLink, Calendar } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        const matchingOrder = orders.find((o: Order) => o.id === unwrappedParams.id);
                ...
                <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                            {order.courierName ? `Shipped via ${order.courierName}` : "Courier info pending"}
                        </p>
                    </div>
                    <Badge variant={order.status === 'delivered' ? 'default' : 'secondary'} className="text-sm px-3 py-1 capitalize">
                        {order.status}
                    </Badge>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="grid gap-1">
                            ...
                            <div className="flex items-center gap-2 p-3 border rounded-md bg-card">
                                <span className="font-mono text-lg flex-1">{order.trackingNumber}</span>
